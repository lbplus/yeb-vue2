<template>
  <div>
    ...
    <el-button type="danger" style="margin-top=10px" :disabled="this.multipleSelection.length == 0" @click="deleteMany">
      批量删除
    </el-button>
    ...
    <el-dialog title="编辑职位" :visible.sync="dialogVisible" width="30%">
      <div>
        <el-tag>职位名称</el-tag>
        <el-input v-model="updatePos.name" size="small" class="updataPosInput"></el-input>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, data) {
      Object.assign(this.updatePos, data);//这里是拷贝，不是直接赋值
